...
sys.path.insert(0, '..')
from cgd_optimizer import CGD
import numpy as np
from bilinear_game.bilinear_game import bilinear_game
from torch.utils.tensorboard import SummaryWriter
from bilinear_game.networks_bilinear import policy
import time
...
optim = CGD(p1.parameters(),p2.parameters(), lr = 0.1)
...
for t_eps in range(num_episode):
    ...
    if t_eps%100==0:
        print(t_eps)
        torch.save(p1.state_dict(), '../' + folder_location + experiment_name + 'model/agent1_' + str(t_eps) + ".pth")
        torch.save(p2.state_dict(), '../' + folder_location + experiment_name + 'model/agent2_' + str(t_eps) + ".pth")