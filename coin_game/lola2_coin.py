...
from lola_optimizer import LOLA2
...
def main():
	...
	job_path = r'../coin_game/job_coin2.yaml'
	...
	with open(info_str, 'a') as info_ptr:
		info_ptr.write('This is run number {} of MAVPG (LOLA2). The hyperparameters are:\n\n'.format(args.run_num))
	...
	if args.lola:
		policy_optim = LOLA2(p1.parameters(), p2.parameters(), lr = args.lr_p, device = device, collect_info = True)
	else:
		if not args.rms:
	...
	for epoch in range(args.n_epochs):
		...
		if args.tensorboard:
			...
			writer.add_scalar('Norm/grad_xy(g)_grad_y(f)', grad_x_y_mh, epoch)
			writer.add_scalar('Norm/grad_y(g)', grad_y, epoch)
			writer.add_scalar('Norm/grad_yx(f)_grad_x(g)', grad_y_x_mh, epoch)
