...
def main():
	# training settings
	parser = argparse.ArgumentParser(description = 'GDA coin game')
	...
	parser.add_argument('--lr-p1', type = float, default = 0.5, metavar = 'LR',
						help = 'gda learning rate for actors (default: 0.5)')
	...
	experiment_name = '/gda/run_' + str(args.run_num)#+'_gda_lr='+str(args.lr_p)+'_'+args.activation_function+'_'+args.policy
	...
	if args.logs:
		logs_file_path = env_location + experiment_name + '/coin_gda_run_' + str(args.run_num) + '.txt'
		f_ptr = open(logs_file_path, 'a')
		s = 'This is run number {} of GDA\n\n'.format(args.run_num)
		f_ptr.write(s)
	...
	if args.logs:
		...
		f_ptr.write(s)
		np_file = env_location + experiment_name + '/coin_gda_run_' + str(args.run_num) + '_lr=' + str(args.lr_p1) + '_' + str(args.lr_p2) + '_stuff.npz'
	...
	ax.plot(np.array(arr_pR_cR), label = 'GDA, RED player picks RED coin, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(arr_pR_cB), label = 'GDA, RED player picks BLUE coin, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(arr_pB_cR), label = 'GDA, BLUE player picks RED coin, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(arr_pB_cB), label = 'GDA, BLUE player picks BLUE coin, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(arr_draw), label = 'GDA, DRAW, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location + experiment_name + '/coin_gda_run_' + str(args.run_num) + '_lr=' + str(args.lr_p1) + '_' + str(args.lr_p2) + '_pick_rate.png')
	...
	plt.xticks(fontsize = 15)
	plt.yticks(fontsize = 15)
	ax.plot(np.array(avg_rew_1), label = 'GDA, Avg rew per epoch Ag1 (RED), lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(avg_rew_2), label = 'GDA, Avg rew per epoch Ag2 (BLUE), lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location + experiment_name + '/coin_gda_run_' + str(args.run_num) + '_lr=' + str(args.lr_p1) + '_' + str(args.lr_p2) + '_avg_rew_per_epoch.png')
