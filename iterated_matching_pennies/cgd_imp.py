...
def main():
	# training settings
	parser = argparse.ArgumentParser(description = 'MAVPG iterated matching pennies')
...
#		env = iterated_matching_pennies(200)
	...
	if args.logs:
		logs_file_path = env_location+experiment_name+'/imp_mavpg_lr='+str(args.lr_p)+'.txt'
	...
	for epoch in range(args.n_epochs):
		...
		# action is either heads or tails
		if args.tensorboard:
	...
	if args.logs:
		...
		f_ptr.write(s)
		np_file = env_location+experiment_name+'/imp_mavpg_lr='+str(args.lr_p)+'_rew_prob_h_t_time.npz'
	...
	ax.plot(np.array(prob_h_1), label = 'MAVPG, Prob_H1, lr = {}'.format(args.lr_p))
	ax.plot(np.array(prob_t_1), label = 'MAVPG, Prob_T1, lr = {}'.format(args.lr_p))
	ax.plot(np.array(prob_h_2), label = 'MAVPG, Prob_H2, lr = {}'.format(args.lr_p))
	ax.plot(np.array(prob_t_2), label = 'MAVPG, Prob_T2, lr = {}'.format(args.lr_p))
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location+experiment_name+'/imp_mavpg_lr='+str(args.lr_p)+'_prob_h_t.png')
	...
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location+experiment_name+'/imp_mavpg_lr='+str(args.lr_p)+'_avg_rew_per_epoch.png')
