...
from iterated_matching_pennies.networks_imp import *
from iterated_matching_pennies.imp_game import iterated_matching_pennies
...
def main():
	# training settings
	parser = argparse.ArgumentParser(description = 'GDA iterated matching pennies')
	...
	experiment_name = '/gda_lr=' + str(args.lr_p1) + '_' + str(args.lr_p2) + '/run_' + str(args.run_num) + '_' + args.activation_function + '_' + args.policy
...
#		env = iterated_matching_pennies(200)
	...
	if args.logs:
		logs_file_path = env_location+experiment_name+'/imp_gda_lr='+str(args.lr_p1)+'_'+str(args.lr_p2)+'.txt'
	...
	for epoch in range(args.n_epochs):
		...
		if args.tensorboard:
			writer.add_scalar('Mean_prob_heads/Agent1', pi1_a_s.data[:, 0].mean(), epoch)
			writer.add_scalar('Mean_prob_tails/Agent1', pi1_a_s.data[:, 1].mean(), epoch)
			writer.add_scalar('Mean_prob_heads/Agent2', pi2_a_s.data[:, 0].mean(), epoch)
			writer.add_scalar('Mean_prob_tails/Agent2', pi2_a_s.data[:, 1].mean(), epoch)
	...
	if args.logs:
		...
		f_ptr.write(s)
		np_file = env_location+experiment_name+'/imp_gda_lr='+str(args.lr_p1)+'_'+str(args.lr_p2)+'_rew_prob_h_t_time.npz'
	...
	ax = plt.subplot()
	ax.clear()
	fig.suptitle('Probabilities of heads and tails v/s #iterations/epochs (repeated steps = {})'.format(args.repeated_steps), fontsize = 25)
	plt.xlabel('$Iterations/Epochs$', fontsize = 20)
	plt.ylabel('$Probability$', fontsize = 20)
	plt.xticks(fontsize = 15)
	plt.yticks(fontsize = 15)
	ax.plot(np.array(prob_h_1), label = 'GDA, Prob_H1, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(prob_t_1), label = 'GDA, Prob_T1, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(prob_h_2), label = 'GDA, Prob_H2, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	ax.plot(np.array(prob_t_2), label = 'GDA, Prob_T2, lr1 = {}, lr2 = {}'.format(args.lr_p1, args.lr_p2))
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location+experiment_name+'/imp_gda_lr='+str(args.lr_p1)+'_'+str(args.lr_p2)+'_prob_h_t.png')
	...
	plt.legend(loc = 'upper right')
	plt.grid()
	plt.savefig(env_location+experiment_name+'/imp_gda_lr='+str(args.lr_p1)+'_'+str(args.lr_p2)+'_avg_rew_per_epoch.png')
